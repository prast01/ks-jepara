                    ...
                    <div class="card card-shadow mb-4">
                        <div class="card-header border-0">
                            <div class="custom-title-wrap bar-primary">
                                <div class="custom-title">INDEKS KELUARGA SEHAT - DESA <?php echo $dat['kelurahan']; ?></div>
                            </div>
                        </div>
                        <div class="card-body">
                            <div class="pt-3 pb-4">
                                <div class="table-responsive">
                                    <table class="table table-bordered table-striped" cellspacing="0">
                                        ...
                                        <tr>
                                            ...
                                            <th rowspan="2" class="align-middle text-center">No RW</th>
                                        ...
                                        <tbody>
                                            <?php
                                                ...
                                                $total_kk = 0;
                                                if ($level == 1) {
                                                    $sql= $db->prepare("SELECT * FROM tb_kk WHERE kecamatan=? AND kelurahan=? AND sts_valid='1' GROUP BY rw ORDER BY rw ASC");
                                                    $sql->execute([$_GET['kecamatan'], $_GET['desa']]);
                                                } else {
                                                    $sql= $db->prepare("SELECT * FROM tb_kk WHERE puskesmas=? AND kelurahan=? AND sts_valid='1' GROUP BY rw ORDER BY rw ASC");
                                                    $sql->execute([$id_puskesmas, $_GET['desa']]);
                                                }
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    <?php
                                                        if ($level == 1) {
                                                            $link = "?page=laporan&subpage=iks_wilayah&kecamatan=".$_GET['kecamatan']."&desa=".$key2['kelurahan']."&rw=".$key2['rw'];
                                                        } else {
                                                            $link = "?page=laporan&subpage=iks_wilayah&desa=".$key2['kelurahan']."&rw=".$key2['rw'];
                                                        }
                                                    ?>
                                                    <a href="<?php echo $link; ?>"><?php echo $key2['rw']; ?></a>
                                                </td>
                                                <?php

                                                    $sql= $db->prepare("SELECT * FROM tb_view_iks WHERE kelurahan=? AND puskesmas=? AND rw=?");
                                                    $sql->execute([$key2['kelurahan'], $key2['puskesmas'], $key2['rw']]);
                                            ...
                                            <tr>
                                                <th colspan="2">% Cakupan Desa <?php echo $dat['kelurahan']; ?></th>
                                                <?php
                                                    foreach ($data as $key) {
