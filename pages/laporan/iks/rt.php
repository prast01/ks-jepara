                    ...
                    <div class="card card-shadow mb-4">
                        <div class="card-header border-0">
                            <div class="custom-title-wrap bar-primary">
                                <div class="custom-title">INDEKS KELUARGA SEHAT - DESA <?php echo $dat['kelurahan']; ?> RW <?php echo $_GET['rw']; ?> RT <?php echo $_GET['rt']; ?></div>
                            </div>
                        </div>
                        <div class="card-body">
                            <div class="pt-3 pb-4">
                                <div class="table-responsive">
                                    <table class="table table-bordered table-striped" cellspacing="0">
                                        ...
                                        <tr>
                                            ...
                                            <th rowspan="2" class="align-middle text-center">Nama KK</th>
                                        ...
                                        <tbody>
                                            <?php
                                                ...
                                                $total_kk = 0;
                                                if ($level == 1) {
                                                    $sql= $db->prepare("SELECT * FROM tb_kk WHERE kecamatan=? AND kelurahan=? AND rw=? AND rt=? AND sts_valid='1' ORDER BY no_kk ASC");
                                                    $sql->execute([$_GET['kecamatan'], $_GET['desa'], $_GET['rw'], $_GET['rt']]);
                                                } else {
                                                    $sql= $db->prepare("SELECT * FROM tb_kk WHERE puskesmas=? AND kelurahan=? AND rw=? AND rt=? AND sts_valid='1' ORDER BY no_kk ASC");
                                                    $sql->execute([$id_puskesmas, $_GET['desa'], $_GET['rw'], $_GET['rt']]);
                                                }
                                                $data2 = $sql->fetchAll(PDO::FETCH_ASSOC);
                                                $total_kk = count($data2);
                                            ...
                                            <tr>
                                                <td><?php echo $no; ?></td>
                                                <td>
                                                    <span class="text-info" style="cursor:pointer" onclick="modalIks('IKS INTI', <?php echo $key2['no_kk']; ?>, 'iksInti')"><?php echo $key2['nama_kk']; ?></span>
                                                </td>
                                                <?php

                                                    $sql= $db->prepare("SELECT * FROM tb_view_iks WHERE kelurahan=? AND puskesmas=? AND rw=? AND rt=? AND no_kk=?");
                                                    $sql->execute([$key2['kelurahan'], $key2['puskesmas'], $key2['rw'], $key2['rt'], $key2['no_kk']]);
                                                    ...
                                                    foreach ($data as $key) {
                                                ?>
                                                <td class="align-middle text-center"><?php echo $data3[0]['i'.$key['no_urut']]; ?></td>
                                                <?php
                                                    }
                                                ?>
                                            ...
                                            <tr>
                                                <th colspan="2">% Cakupan Desa <?php echo $dat['kelurahan']; ?> RW <?php echo $_GET['rw']; ?> RT <?php echo $_GET['rt']; ?></th>
                                                <?php
                                                    foreach ($data as $key) {
